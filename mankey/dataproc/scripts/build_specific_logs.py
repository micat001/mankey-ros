...
parser.add_argument(
    "--pdc_root",
    type=str,
    default="/home/wei/Coding/correspondence",
    help="The root path of pytorch-dense-correspondence",
)
parser.add_argument(
    "--composite_dataset",
    type=str,
    default="shoe_train_all_shoes.yaml",
...
def process_singleobj_dataset(
    config_full_path: str, logs_root_path: str
) -> (List[str], List[str]):
    """
...
def process_composite_dataset(config_fullpath: str, output_txt_path: str):
    ...
    with open(config_fullpath, "r") as stream:
        try:
            ...
            singleobj_config_path = composite_config[
                "single_object_scenes_config_files"
            ]
            for config in singleobj_config_path:
                singleobj_config_fullpath = os.path.join(
                    singleobj_dataset_config_root, config
                )
                train_list, test_list = process_singleobj_dataset(
                    singleobj_config_fullpath, logs_root_path
                )
...
def main():
    ...
    composite_dataset_fullpath = os.path.join(
        composite_dataset_config_root, args.composite_dataset
    )
