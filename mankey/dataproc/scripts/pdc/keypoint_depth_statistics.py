...
def main():
    ...
    for i in range(len(dataset)):
        ...
        for j in range(n_keypoint):
            min_keypoint_depth = min(
                min_keypoint_depth, processed_entry.keypoint_xy_depth[2, j]
            )
            max_keypoint_depth = max(
                max_keypoint_depth, processed_entry.keypoint_xy_depth[2, j]
            )
