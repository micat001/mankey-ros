...
parser.add_argument(
    "--annotation_type",
    type=str,
    default="shoe_standard",
    help="Which type of annotation need to be processed",
)
parser.add_argument(
    "--save_relative_path",
    type=str,
    default="shoe_6_keypoint_image.yaml",
...
parser.add_argument(
    "--log_root_path",
    type=str,
    default="/home/wei/data/pdc/logs_proto",
...
def process_annotation_dict(annotation_dict, keypoint_name_list: List[str]):
    ...
    mesh2img_command = (
        "python %s --keypoint_yaml_path %s --scene_log_path %s --output_yaml_path %s"
        % (
            keypoint_mesh2img_exe_file,
            tmp_keypoint_yaml_path,
            scene_root_path,
            tmp_imgkeypoint_yaml_path,
        )
    )
    ...
    img_keypoint_output_path = os.path.join(
        scene_processed_path, args.save_relative_path
    )
    bbox_command = (
        "python %s --scene_root_path %s --pose_yaml_path %s --output_yaml_path %s"
        % (
            bbox_exe_file,
            scene_root_path,
            tmp_imgkeypoint_yaml_path,
            img_keypoint_output_path,
        )
    )
    print(bbox_command)
...
def main():
    ...
    for annotation_yaml_file in os.listdir(annotation_dir):
        ...
        for annotation_dict in annotation_yaml_list:
            ...
            if (
                "annotation_type" not in annotation_dict
                or annotation_dict["annotation_type"] != args.annotation_type
            ):
                continue
            ...
            if scene_name in processed_scene:
                print(
                    "The scene %s is annotated twice for %s"
                    % (scene_name, args.annotation_type)
                )
                continue
            else:
