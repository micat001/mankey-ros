...
parser.add_argument(
    "--scene_image_keypoint_output_path",
    type=str,
...
def main():
    ...
    for folder in os.listdir(log_root):
        ...
        if os.path.isdir(scene_log_path):
            keypoint_yaml_path = os.path.join(
                scene_processed_path, args.scene_mesh_keypoint_path
            )
            output_yaml_path = os.path.join(
                scene_processed_path, image_keypoint_tempfile
            )
    ...
    bbox_command = (
        "python %s --log_root_path %s --pose_yaml_filename %s --output_yaml_name %s"
        % (
            bbox_exe_file,
            log_root,
            image_keypoint_tempfile,
            args.scene_image_keypoint_output_path,
        )
    )
    print(bbox_command)
    ...
    # Some info
    print(
        "Processed logs (scenes) %d, while %d scene does not have keypoint."
        % (processed_counter, skip_counter)
    )
