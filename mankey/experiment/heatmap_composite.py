...
def construct_dataset(
    ...
    if is_train:
        db_config.config_file_path = (
            "/home/wei/Coding/mankey/config/mugs_up_with_flat_logs.txt"
        )
    else:
        db_config.config_file_path = (
            "/home/wei/Coding/mankey/config/mugs_up_with_flat_test_logs.txt"
        )
...
def construct_network():
    ...
    net_config.depth_per_keypoint = (
        3  # For integral heatmap, depthmap and regress heatmap
    )
...
def train(checkpoint_dir: str, start_from_ckpnt: str = "", save_epoch_offset: int = 0):
    ...
    loader_train = DataLoader(
        dataset=dataset_train, batch_size=32, shuffle=True, num_workers=4
    )
    ...
    for epoch in range(n_epoch):
        ...
        for idx, data in enumerate(loader_train):
            ...
            depthmap_pred = raw_pred[
                :, net_config.num_keypoints : 2 * net_config.num_keypoints, :, :
            ]
            ...
            (
                keypoint_xy_pred_heatmap,
                _,
            ) = predict.heatmap2d_to_normalized_imgcoord_argmax(regress_heatmap)
            xy_error_heatmap = float(
                weighted_l1_loss(
                    keypoint_xy_pred_heatmap[:, :, 0:2],
                    keypoint_xy_depth[:, :, 0:2],
                    keypoint_weight[:, :, 0:2],
                ).item()
            )
            if idx % 100 == 0:
                ...
                print(
                    ...
                    train_config.depth_image_scale * depth_error / len(xy_depth_pred),
                )
                print(
